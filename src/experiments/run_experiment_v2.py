...
def run_master_orchestrator_v2():
    ...
    log_file = os.path.join(RESULTS_DIR, f"log_master_V2_{timestamp}.txt")
    csv_file = os.path.join(RESULTS_DIR, f"resultados_master_V2_{timestamp}.csv")
    sys.stdout = Logger(log_file)
    
    print(f"{'='*75}\nORQUESTRADOR V2: SUPER FEATURES FÍSICAS + MODELOS HÍBRIDOS (1s Windows)\n{'='*75}")
    ...
    for dataset_name, conditions in ALL_DATASETS.items():
        ...
        for task in TASKS:
            ...
            for fold_name, test_cond_val in iterable_conditions:
                ...
                # 1. Carregamento dos Dados (agora janelas de 1 segundo = menos amostras)
    ...
    if master_results:
        ...
        print(f"\n\n[SUCESSO] Tabela Master V2 exportada para: {csv_file}")
