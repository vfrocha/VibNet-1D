...
# Importamos a NOVA função de extração de frequência
from src.features.extractors import extract_freq_features
...
def run_statistical_freq_experiment():
    # Cria a pasta results se não existir
    os.makedirs(RESULTS_DIR, exist_ok=True)
    
    # Configura os arquivos de saída com marcadores de "freq" (Frequência)
    timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
    log_file = os.path.join(RESULTS_DIR, f"log_exp03_freq_{DATASET}_{timestamp}.txt")
    csv_file = os.path.join(RESULTS_DIR, f"resultados_exp03_freq_{DATASET}_{timestamp}.csv")
    
    # Redireciona o print para o terminal E para o arquivo de log
    sys.stdout = Logger(log_file)
    
    print(f"\n{'='*50}")
    print(f"INICIANDO EXPERIMENTO 3: STATISTICAL (Frequency Domain)")
    ...
    for test_cond in CONDITIONS:
        ...
        # 2. Extrai as métricas espectrais (FFT)
        # Note que estamos chamando extract_freq_features em vez de extract_time_features
        X_train_feat = extract_freq_features(X_train_raw, fs=12000)
        X_test_feat = extract_freq_features(X_test_raw, fs=12000)

        # 3. Treina e avalia os modelos com GridSearch
        
        # --- Random Forest ---
        print("\n  [Iniciando Random Forest - Frequência]")
    ...
    if results:
        ...
        print("\n\n" + "="*50)
        print("RELATÓRIO FINAL - DOMÍNIO DA FREQUÊNCIA")
        print("="*50)
        ...
        print(f"\n[Sucesso] Tabela de resultados de Frequência exportada para: {csv_file}")

if __name__ == "__main__":
    run_statistical_freq_experiment()
